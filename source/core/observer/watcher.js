...
export default class Watcher {
  ...
  //所有的操作都会在 newDeps 中完成，然后在清空的时候会把 newDeps 清空，然后把 newDeps 中的数据归档到 deps，就是所谓的二次提交
  newDeps: Array<Dep>;
  newDepIds: SimpleSet;

  before: ?Function;  //在定义 Vue 构造函数的时候，传入的 watch
  getter: Function;  //就是 渲染函数(模板或组件的渲染) 或 计算函数(watch)，是最终的计算方法
  ...
  update () {
    ...
    if (this.lazy) {   //主要用于计算属性，一般用于求值计算
      this.dirty = true
    } else if (this.sync) {    //同步，主要用于SSR，同步就表示立即计算
      this.run()
    } else {
      queueWatcher(this)    //一般用于浏览器中的同步运行
    }
  }
