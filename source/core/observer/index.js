...
export class Observer {
  ...
  constructor (value: any) {
    ...
    if (Array.isArray(value)) {
      ...
      this.observeArray(value)  //编辑数组的元素，进行递归 observe
    } else {
 ...
 * 
 * 将传入的数据 value 变成响应式的对象
 * 
 * 算法描述：
 * - 先看 对象是否含有 __ob__，并且是 Observer 的实例( vue 中响应式对象的标记 )
 * - 有，忽略
...
export function defineReactive (
  ...
  let childOb = !shallow && observe(val)  //拿到的是响应式对象背后的 __ob__

  Object.defineProperty(obj, key, {
    enumerable: true,
    configurable: true,
    get: function reactiveGetter () {
      //保证了如果已经定义的 get 方法可以被继承下来，不会丢失
      const value = getter ? getter.call(obj) : val

      //如果全局范围内有 watcher ，就进行依赖收集
      if (Dep.target) {
        dep.depend()  //用 dep 来进行依赖收集，关联的是当前属性
